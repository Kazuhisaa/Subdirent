...
<head>
  ...
  <style>
    :root { --pdogreen: #184d2b; --pdogold: #ffc107; }
    body { overflow-x: hidden; }
    .d-flex-full { display: flex; min-height: 100vh; }
    .sidebar { width: 60px; transition: width 0.3s; overflow: hidden; background-color: #f8f9fa; border-right: 1px solid #dee2e6; padding-top: 10px; }
    .sidebar.expanded { width: 220px; }
    .sidebar .btn { display: flex; align-items: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 10px; transition: all 0.3s; }
    .sidebar .btn i { margin-right: 0; transition: margin 0.3s; }
    .sidebar.expanded .btn i { margin-right: 10px; }
    .sidebar.expanded .btn span { display: inline; }
    .sidebar .btn span { display: none; }
    .content { flex-grow: 1; padding: 20px; transition: margin-left 0.3s; }
    #sidebarToggle { position: absolute; top: 10px; left: 10px; z-index: 1000; background-color: var(--pdogreen); border: none; color: white; }
    
    /* Fix sidebar text visibility for inactive links */
    .sidebar .btn:not(.text-white) { color: #184d2b; background-color: #e9ecef; }
    .sidebar .btn.text-white { color: #fff !important; background-color: var(--pdogreen); }
  </style>
</head>
<body class="bg-light">
  ...
  <div class="d-flex-full position-relative">
    ...
    <div id="sidebar" class="sidebar d-flex flex-column p-2">
      <a href="{{ route('admin.dashboard') }}" class="btn fw-semibold text-white">
